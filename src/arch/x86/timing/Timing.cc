...
{
	//Get the emulator and esim instance
	Emulator *emulator = Emulator::getInstance();
	esim::Engine *esim_engine = esim::Engine::getInstance();
    ...
    //Get the SI Emulatr instance to count the number of ndranges running
	SI::Emulator *si_emulator = SI::Emulator::getInstance();
...
void Timing::RegisterOptions()
{
	...
	//Option --x86-opencl-fast-forward
	command_line->RegisterBool("--x86-opencl-fast-forward", Timing::opencl_fast_forward,
